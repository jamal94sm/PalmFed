...
class compnet_fedpalm(torch.nn.Module):
    '''
    ...
    def __init__(self, num_classes):
        super(compnet_fedpalm, self).__init__()
        ...
        self.fc_brand = torch.nn.Linear(9708, 512)  # <---
        self.drop = torch.nn.Dropout(p=0.25)
        self.arclayer_brand = ArcMarginProduct(512, self.num_classes, s=30, m=0.5, easy_margin=False)

        self.evi_brand = torch.nn.Linear(512,self.num_classes)
        self.relu = nn.ReLU()

    def forward(self, x, y=None, other_out=None):
        x1 = self.cb1(x)
        x2 = self.cb2(x)
        x3 = self.cb3(x)
        ...
        if other_out == None:
            x = self.fc_brand(out)
            fe = x / torch.norm(x, p=2, dim=1, keepdim=True)
        else:
            x = self.fc_brand(other_out)
            fe = x / torch.norm(x, p=2, dim=1, keepdim=True)

        # evi = self.evi_brand(x)

        x = self.drop(x)
        x = self.arclayer_brand(x, y)

        # return x, fe, self.relu(evi)
        return x, fe, out
...
class compnet_fedpalm_2(torch.nn.Module):
    '''
    CompNet = CB1//CB2//CB3 + FC + Dropout + (angular_margin) Output\n
    https://ieeexplore.ieee.org/document/9512475
    '''
    ...
    def __init__(self, num_classes):
        super(compnet_fedpalm_2, self).__init__()
    ...
    def forward(self, x, y=None, other_out=None):
        ...
        if other_out == None:
            ...
            # out = (out + other_out) / 2
            # out = out * 0.8 + other_out * 0.2
            # out, weight = self.atten(out, other_out)
            out = self.a * out + self.b * other_out
            x = self.fc_brand(out)
            fe = x / torch.norm(x, p=2, dim=1, keepdim=True)
...
class compnet_bn(torch.nn.Module):
    '''
    CompNet = CB1//CB2//CB3 + FC + Dropout + (angular_margin) Output\n
    https://ieeexplore.ieee.org/document/9512475
    '''
    def __init__(self, num_classes):
        super(compnet_bn, self).__init__()   
...
if __name__== "__main__" :
    ...
    other_out = torch.randn(1,9708)

    net = compnet_fedpalm_2(600)
    # temp_inp = torch.randn()
    for key in net.state_dict().keys():
        if 'arc' not in key:
            print(key)
    
    out = net(inp,other_out=other_out)
