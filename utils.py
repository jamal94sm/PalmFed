...
def train_compnet_epoch(model, loader, criterion, optimizer, device,
    ...
    StyleConsistencyLoss:
      ...
      Enforces that embeddings are invariant to domain style transfer.
      Works for both AugmentedDataset (spatial aug pairs) and
      FFTAugmentedDataset (FFT-styled pairs) — both return ([orig, aug], label).

      For FFTAugmentedDataset:
        aug_idx=0 → emb(orig) vs emb(spatial_aug(orig))
        aug_idx≥1 → emb(orig) vs emb(fft_aug_styled_as_other_domain)
      The FFT pair signal directly enforces that the backbone produces
      the same embedding for the original 940nm image and the same image
      styled to look like 460nm — cross-domain feature invariance.
    ...
    for data, labels in loader:
        ...
        # both AugmentedDataset and FFTAugmentedDataset return ([orig, aug], label)
