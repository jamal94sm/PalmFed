...
def train_compnet_epoch(model, loader, criterion, optimizer, device,
    ...
    for imgs, labels in loader:
        ...
        if use_supcon or use_center:
            ...
            if use_supcon:
                # SupConLoss requires ≥2 same-label samples in the batch to
                # form positive pairs. With many classes and small batches this
                # is not guaranteed — skip rather than produce NaN loss.
                label_counts = torch.bincount(labels, minlength=labels.max().item()+1)
